...
export async function configureHooks(options: ConfigureHooksOptions): Promise<string[]> {
...
export async function clearHooks(options: ClearHooksOptions): Promise<string[]> {
  const changed: string[] = [];
  for (const producer of options.producers) {
    if (producer === "codex") {
...
export async function configureCodexHooks(
  scope: ConfigScope,
  events: HookEvent[],
  cwd: string
): Promise<string[]> {
  const { hooksPath, configPath } = codexPaths(scope, cwd);
  const hookConfig = await readJson<HookConfig>(hooksPath, {});
...
export async function clearCodexHooks(scope: ConfigScope, cwd: string): Promise<string[]> {
...
export async function configureClaudeHooks(
  ...
  const hookConfig = await readJson<HookConfig>(settingsPath, {});
  hookConfig.hooks = hookConfig.hooks ?? {};
...
export async function clearClaudeHooks(scope: ConfigScope, cwd: string): Promise<string[]> {
  return clearManagedHooksFile(claudeSettingsPath(scope, cwd), "claude");
}

export async function configureAidenHooks(
  scope: ConfigScope,
  events: HookEvent[],
  cwd: string
): Promise<string[]> {
...
export async function clearAidenHooks(scope: ConfigScope, cwd: string): Promise<string[]> {
...
function ensureTomlFeatureFlags(content: string, flags: Record<string, string>): string {
...
async function clearManagedHooksFile(path: string, producer: EndpointKind): Promise<string[]> {
  if (!await pathExists(path)) {
    return [];
  }
  const hookConfig = await readJson<HookConfig>(path, {});
...
function removeManagedHookCommands(config: HookConfig, producer: EndpointKind): boolean {
  if (!config.hooks) {
    return false;
  }
  let removed = false;
...
function isManagedHookCommand(command: string, producer: EndpointKind): boolean {
  return command.includes(`hook --producer ${producer}`)
    && (command.includes("--event stop") || command.includes("--event post-tool-use"));
}

function codexPaths(scope: ConfigScope, cwd: string): { hooksPath: string; configPath: string } {
...
function claudeSettingsPath(scope: ConfigScope, cwd: string): string {
  return scope === "global"
    ? join(homedir(), ".claude", "settings.json")
    : join(cwd, ".claude", "settings.local.json");
}

function aidenSettingsPath(scope: ConfigScope, cwd: string): string {
  return scope === "global"
