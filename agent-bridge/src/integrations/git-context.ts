...
import { MAX_DIFF_CHARS } from "../core/constants.ts";
import type { GitContext } from "../core/types.ts";
import { truncateText } from "../utils/fs.ts";
...
async function runGit(cwd: string, args: string[]): Promise<{ exitCode: number; stdout: string; stderr: string }> {
  try {
...
async function formatUntrackedFile(cwd: string, file: string): Promise<string> {
...
function formatSyntheticNewFile(file: string, lines: string[]): string {
  return [
...
function shouldSkipUntrackedFile(file: string): boolean {
