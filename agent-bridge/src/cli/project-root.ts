...
import type { ConfigScope } from "../core/types.ts";
import { pathExists } from "../utils/fs.ts";

export async function resolveHookConfigCwd(scope: ConfigScope, cwd: string): Promise<string> {
  if (scope !== "project") {
    return cwd;
  }
...
export async function resolveCodexWorktreeProjectRoot(cwd: string): Promise<string | null> {
...
async function findNearestGitFile(start: string): Promise<string | null> {
