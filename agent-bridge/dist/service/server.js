...
import { PID_FILE, PORT_FILE, STATE_DIR, PRODUCER_LABELS, bridgeGateTimeoutMs } from "../core/constants.js";
import { dashboardHtml } from "../dashboard/page.js";
import { runBridge } from "../bridge/runner.js";
import { loadBridgeRuns, markInterruptedBridgeRuns } from "../bridge/run-state.js";
import { ensureTerminalLogTail, readTerminalLog, subscribeTerminalLog, terminalLogPath } from "../terminal/logs.js";
import { handleTerminalWebSocket } from "../terminal/websocket.js";
import { ensureDir, pathExists, readJson, readText, writeJson, writeText } from "../utils/fs.js";
export async function runServiceForeground() {
...
export async function printDashboardUrl() {
    await startService();
    const status = await serviceStatus();
    console.log(`Dashboard: http://127.0.0.1:${status.port}/dashboard`);
}
export async function submitBridge(envelope) {
    const config = await loadConfig();
...
async function serviceStatus() {
    const config = await loadConfig();
    const state = await readJson(PORT_FILE, {});
...
async function postBridge(port, envelope) {
...
async function streamTerminalLog(response, runId, kind) {
...
function parseTerminalRoute(url) {
    if (!url) {
        return null;
    }
...
function formatConsumerStatus(status) {
    return colorStatus(status, status);
}
function colorStatus(status, rawStatus) {
