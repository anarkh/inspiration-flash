...
import type { DerivedStats, EquipmentId, EquipmentSlot, ItemId } from './game';
...
const IF_EQUIPMENT_CATALOG_DIAG = 'IF_EQUIPMENT_CATALOG_DIAG_V1';

const INITIAL_EQUIPMENT_IDS = [
  'training_blade',
  'patched_headwrap',
  'patched_coat',
  'patched_gloves',
...
function throwEquipmentCatalogDiagnostic(equipmentId: string): never {
  const catalogType = typeof EQUIPMENT;
  const isCatalogObject = catalogType === 'object' && EQUIPMENT !== null;
  const ownKeys = isCatalogObject ? Object.keys(EQUIPMENT) : [];
  const initialPresence = INITIAL_EQUIPMENT_IDS.map((id) => ({
    id,
    present: isCatalogObject
      ? Object.prototype.hasOwnProperty.call(EQUIPMENT, id)
      : false,
  }));
  throw new Error(
    `${IF_EQUIPMENT_CATALOG_DIAG} ${JSON.stringify({
      equipmentId,
      catalogType,
      ownKeyCount: ownKeys.length,
      initialPresence,
      firstKeys: ownKeys.slice(0, 12),
    })}`,
  );
}
...
function getSafeLevel(equipmentId: EquipmentId, level: number): number {
  ...
  if (entry === undefined) {
    throwEquipmentCatalogDiagnostic(equipmentId);
  }
...
export function getEquipmentAttunementResonanceProgress(
  ...
  const uniqueEquippedIds = Array.from(new Set(equippedIds));
