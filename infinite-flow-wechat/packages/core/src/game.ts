...
export type DungeonSeedSource = () => number;
...
// EQUIPMENT catalog moved to ./equipment-catalog to break the game <-> equipment-system runtime cycle.
export { EQUIPMENT };
...
function getArchivedRunRelicIds(state: GameState): RunRelicId[] {
  ...
  return Array.from(new Set(archivedRelicIds.filter(isRunRelicId)));
}
...
function createHiddenTaskSeed(
  ...
  if (!seedSource) {
    throw new TypeError('A seedSource is required when no valid dungeon seed is provided.');
  }

  const generated = seedSource();
  if (!Number.isInteger(generated) || generated < 1 || generated > 0xffff_ffff) {
    throw new RangeError('seedSource must return a positive uint32 seed.');
  }
  return generated;
}
...
export function enterDungeon(
  ...
  const infernoMap = layeredInferno
      ? createInfernoMapSnapshot({
          dungeon,
          seed: createHiddenTaskSeed(options.infernoMapSeed, options.seedSource),
  ...
  const nextState: GameState = {
    ...
    run: {
      ...
      ...(simplifiedEntry
        ...
        : {}),
      ...(simplifiedEntry
        ? { hiddenTaskSeed: createHiddenTaskSeed(options.hiddenTaskSeed, options.seedSource) }
        : {}),
      dungeonId,
...
export function resolveRunRelicArchive(
  ...
  const archivedRelicIds = Array.from(new Set([...getArchivedRunRelicIds(state), relicId]));
  return appendLog(
    {
      ...state,
